...
class EnhancedResultDisplay:
    ...
    def show_crawler_analytics(self, crawler_results: List[Dict]):
        """크롤러 분석 통계 표시"""
        st.markdown("### 📈 크롤러 분석 통계")
        
        if not crawler_results:
            st.info("크롤링 데이터가 없습니다.")
            return
        
        # 성공한 크롤링 결과만 필터링
        successful_results = [r for r in crawler_results if 'error' not in r and r.get('title')]
        
        if not successful_results:
            st.warning("성공한 크롤링 결과가 없습니다.")
            return
        ...
        with col2:
            ...
            type_counts = {}
            for result in successful_results:
                article_type = result.get('type', 'Unknown')
        ...
        with col1:
            ...
            st.metric("수집된 기사", f"{total_articles}개")
        
        with col2:
            unique_sources = len(set(r.get('source', '') for r in successful_results))
            st.metric("활성 소스", f"{unique_sources}개")
        
        with col3:
            avg_processing_time = np.mean([r.get('processing_time', 0) for r in successful_results if 'processing_time' in r])
            st.metric("평균 처리 시간", f"{avg_processing_time:.2f}초")
        
        with col4:
            success_rate = len(successful_results) / len(crawler_results) * 100 if crawler_results else 0
            st.metric("수집 성공률", f"{success_rate:.1f}%")