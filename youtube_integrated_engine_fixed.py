#!/usr/bin/env python3
"""
🚀 완전한 멀티모달 통합 분석 엔진 v3.0 (JSON 직렬화 오류 수정)
...
3. python youtube_integrated_engine_fixed.py 실행
4. 완전한 통합 분석 결과 확인
"""
...
async def main():
    """메인 실행 함수"""
    print("🚀 솔로몬드 완전한 멀티모달 통합 분석 엔진 v3.0 (수정됨)")
    ...
    # 상세 결과 JSON 저장 (수정된 부분)
    detailed_results = {
        "timestamp": timestamp,
        "analysis_info": {
            "engine_version": "v3.0_complete_multimodal_fixed",
            "youtube_support": True,
            "total_sources": total_sources,
            "local_files": len(input_files),
            "youtube_videos": len(youtube_urls)
        },
        "individual_analysis": [r.to_dict() for r in analysis_results],  # 수정된 부분
    ...
    with open(report_file, 'w', encoding='utf-8') as f:
        f.write("💎 솔로몬드 완전한 멀티모달 통합 분석 리포트 v3.0 (수정됨)\n")
