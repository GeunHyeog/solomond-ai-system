...
try:
    ...
    print("[SUCCESS] Advanced monitoring loaded")
    print("[SUCCESS] YouTube processor loaded")
except ImportError as e:
...
try:
    ...
    print("[SUCCESS] Large file handler loaded")
except ImportError as e:
...
try:
    ...
    print("[SUCCESS] Browser automation loaded")
    print("[SUCCESS] MCP browser integration loaded")
...
class SolomondRealAnalysisUI:
    ...
    def run(self):
        ...
        # 탭 기반 UI로 확장 (v2.4) - 종합 상황 분석 탭 추가
        ...
        with tab7:
            # 종합 상황 분석 - 화자별 대화 분석 및 마인드맵
            self.render_comprehensive_situation_analysis()
...
"""
    ...
    def render_mcp_browser_tab(self):
        ...
        if not MCP_BROWSER_AVAILABLE:
            st.warning("MCP 브라우저 기능이 비활성화되어 있습니다.")
            st.info("""
            **MCP 브라우저 자동화 기능:**
            - 지능형 웹 페이지 자동 탐색
            - 실시간 데이터 수집 및 분석
        ...
        with col1:
            ...
            with st.expander("🔧 고급 설정"):
                wait_time = st.slider("페이지 로딩 대기시간 (초)", 1, 10, 3)
                screenshot = st.checkbox("스크린샷 캡처", value=True)
        ...
        with col2:
            ...
            # 상태 표시
            if 'mcp_automation_status' not in st.session_state:
                st.session_state.mcp_automation_status = "대기 중"
            ...
            status_color = {
                ...
                "완료": "🔵",
                "오류": "🔴"
            }.get(st.session_state.mcp_automation_status, "⚪")
            
            st.write(f"{status_color} **상태**: {st.session_state.mcp_automation_status}")
            
            # 통계 정보
            st.metric("처리된 페이지", 0)
            st.metric("수집된 데이터", "0 items")
            st.metric("소요 시간", "0:00")
        
        # 실행 버튼
        if st.button("🚀 자동화 시작", type="primary", disabled=not target_url):
            if target_url:
                try:
                    st.session_state.mcp_automation_status = "실행 중"
                    
                    with st.spinner("MCP 브라우저 자동화 실행 중..."):
                        # MCP 브라우저 자동화 실행
                        if self.mcp_browser:
                            result = self.mcp_browser.execute_automation(
                                url=target_url,
                            ...
                            if result.get('success'):
                                st.session_state.mcp_automation_status = "완료"
                                st.success("✅ 자동화가 성공적으로 완료되었습니다!")
                                ...
                                if result.get('screenshot'):
                                    st.subheader("📸 캡처된 스크린샷")
                                    st.image(result['screenshot'])
                                    
                            else:
                                st.session_state.mcp_automation_status = "오류"
                                st.error(f"❌ 자동화 실패: {result.get('error', '알 수 없는 오류')}")
                        else:
                            # MCP 브라우저가 없는 경우 시뮬레이션
                            time.sleep(2)  # 시뮬레이션 지연
                            st.session_state.mcp_automation_status = "완료"
                            st.success("✅ 시뮬레이션 완료 (실제 MCP 브라우저 연결 필요)")
                            ...
                            st.json({
                                ...
                                "task": automation_task,
                                "status": "simulated",
                                "message": "MCP 브라우저 연결 후 실제 실행 가능"
                            })
        ...
        # 자동화 기록
        with st.expander("📜 자동화 기록"):
            if 'mcp_automation_history' not in st.session_state:
                st.session_state.mcp_automation_history = []
            
            if st.session_state.mcp_automation_history:
                for i, record in enumerate(st.session_state.mcp_automation_history):
                    st.write(f"{i+1}. {record['timestamp']} - {record['url']} - {record['status']}")
            else:
                st.info("아직 자동화 기록이 없습니다.")
    ...
    def render_comprehensive_situation_analysis(self):
        """종합 상황 분석 탭 렌더링 - 4단계 워크플로우"""
        ...
        # 분석 워크플로우 선택
        analysis_step = st.session_state.get('analysis_workflow_step', 1)
    ...
    def _render_step1_source_extraction(self):
        ...
        if st.session_state.get('step1_sources_extracted', False):
            ...
            st.write("소스별 정보가 성공적으로 추출되었습니다.")
